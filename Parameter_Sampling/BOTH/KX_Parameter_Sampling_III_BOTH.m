...
function [DataOut, N] = KX_Parameter_Sampling_III_BOTH(iterations,InputDataStructure,MetConcDataIn,MetNames,seed)
...
%% Extract create reactions header
%KX_Extract_Reaction_Headers(N)
...
F1_MOD = 0.2;	% Multiplicative factor defining the lower bound of the sampling intervals for investigated regulatory interaction
F2_MOD = 5;     % Multiplicative factor defining the upper bound of the sampling intervals for investigated regulatory interaction
...
[ParValMin,ParValMax,vmax_K_indeces] = compParInt(N,F1,F2,F1_MOD,F2_MOD);
...
if ~exist('seed','var')
    seed = 'shuffle'; 
end % if no seed defined, set it to 'shuffle', dependend on current time
...
for c = 1:iterations                    % for every interation
    ...
    [dfodc,Parameters(c,:)] = KX_CalDFODC_BOTH(N,Parameters(c,:),vmax_K_indeces,Fluxes,SFull);
...
%% Data output á la K1 (Janasch et al., 2019)
...
%==========================================================================
function [ParValMin,ParValMax,vmax_K_indeces] = compParInt(N,F1,F2,F1_MOD,F2_MOD)
...
for p = 1:length(ParID)                 % for every parameter
    ...
    par_aux = ParID{p};                 % for parameter p
        ...
        if ((~strcmp(par_aux2,'eq')) && (~strcmp(par_aux2,'max')) && (~strcmp(par_aux2,'PPool')) && (~strcmp(par_aux2,'FUMPool')) && (~strcmp(par_aux2,'COAPool'))) 
            % if parameter p is NOT an equilibrium constant or a Vmax
            % value, therefore a Km/Ki/Ka
               ...
               for i=1:length(S)               % for every metabolite
                   if(strcmp(par_aux2,S{i}))   % if the parameter belongs to 
                    ...
                    if strcmp(par_aux,'KiAMPv6')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiATPv6')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiCITv6')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiGLXv6')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiRuBPv6')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiAMPv7')
                       ParValMin(p) = 1*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 1*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiAMPv8')
                       ParValMin(p) = 0.05*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 0.05*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiNADPHv8')
                       ParValMin(p) = 3*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 3*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KiCITv8')
                       ParValMin(p) = 5*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 5*F2_MOD; % set upper bound around concentration used in essay
                    elseif strcmp(par_aux,'KaGAPv8')
                       ParValMin(p) = 0.5*F1_MOD; % set lower bound around concentration used in essay
                       ParValMax(p) = 0.5*F2_MOD; % set upper bound around concentration used in essay   
                    elseif strcmp(par_aux,'Hillv8')
                       ParValMin(p) = 1; % set lower bound
                       ParValMax(p) = 2; % set upper bound
                    elseif strcmp(par_aux,'Hillv10')
                       ParValMin(p) = 1; % set lower
                       ParValMax(p) = 4; % set upper
                    elseif strcmp(par_aux,'Hillv6')
                       ParValMin(p) = 1; % set lower
                       ParValMax(p) = 2; % set upper
                    elseif strcmp(par_aux,'Hillv7')
                       ParValMin(p) = 1; % set lower
                       ParValMax(p) = 2; % set upper
                    end
               end
